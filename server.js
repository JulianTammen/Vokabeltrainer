...
const VOCAB_DB   = process.env.NOTION_VOCAB_DB  || '3ad6ab13-4ee4-80c8-b3d5-f6d49d311c41';
  ...
  for (const b of blocks) {
    ...
    switch (type) {
      ...
      case 'callout': {
        ...
        // Callouts, die nur eine Unterseite enthalten, werden zur reinen Navigationskarte.
        if (!text && kids) {
          html += kids;
        } else {
      ...
      case 'child_database': {
        html += navCard(b.child_database.title || 'Vokabeln', 'data-tab="vokabeln"');
        break;
      }
...
async function loadVocab() {
  ...
  do {
    ...
    const data = await notion('/databases/' + VOCAB_DB + '/query', {
      method: 'POST',
      body: JSON.stringify(body),
    });
    for (const page of data.results) {
      const v = readVocab(page);
      if (v.ar && v.de) out.push(v);
    }
    cursor = data.has_more ? data.next_cursor : null;
  } while (cursor);

  out.sort((a, b) => a.ar.localeCompare(b.ar, 'es'));
  return out;
}
