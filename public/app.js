...
function renderAll() {
  ...
  if (!opts.arten) opts.arten = new Set();   // leer = alle Wortarten
...
function goBack() {
  ...
  if (tab === 'grammatik' && stack.length > 1) {
    stack.pop();
    renderGrammatik();
    window.scrollTo(0, 0);
    return true;
...
// Vom linken Rand nach rechts wischen geht eine Ebene zurück — wie in iOS.
(function () {
  var x0 = 0, y0 = 0, t0 = 0, tracking = false;
  var EDGE = 34;      // nur ganz am Rand starten
  var MIN_X = 60;     // so weit muss gewischt werden
  var MAX_Y = 50;     // so weit darf es senkrecht abweichen
  ...
  document.addEventListener('touchstart', function (e) {
    ...
    // Waagerecht scrollbare Bereiche nicht kapern.
    if (e.target.closest && e.target.closest('.table-wrap, .chips, input, .vcard')) return;
  ...
  document.addEventListener('touchend', function (e) {
    ...
    if (dx > MIN_X && dy < MAX_Y && dx > dy * 2 && Date.now() - t0 < 900) goBack();
  }, { passive: true });
})();
    ...
    '<div class="panel">' +
      '<div class="field">' +
        ...
        '<button class="dirtoggle" id="dir">' +
          ...
          '<span class="arrow" data-rev="' + (opts.dir === 'ar2de' ? '1' : '0') + '">' +
      ...
      '<div class="field">' +
        ...
        '<div class="chips">' +
          '<button class="chip" data-art="*" data-on="' + (opts.arten.size === 0 ? '1' : '0') + '">Alle' +
