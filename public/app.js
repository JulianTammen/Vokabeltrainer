...
var opts = { dir: 'de2ar', arten: null, lekt: null, irr: false, amount: 'Alle' };
var session = null;
...
function renderSetup() {
  ...
  el.t.querySelectorAll('.chip[data-art]').forEach(function (c) {
    c.addEventListener('click', function () {
      var v = c.dataset.art;
      if (opts.arten.has(v)) opts.arten.delete(v); else opts.arten.add(v);
      renderSetup();
    });
  });
...
function startSession() {
  ...
  session = { queue: queue, total: queue.length, done: 0, misses: {}, history: [] };
  hasStarted = true;
  drawCard();
}
...
function drawCard() {
  ...
  var missTotal = Object.keys(session.misses).length;
  var revealed = false;

  var ticks = '';
  for (var i = 0; i < session.total; i++) {
    var s = i < session.done ? 'done' : (i === session.done ? 'now' : 'open');
  ...
  el.t.innerHTML =
    '<div class="study">' +
      ...
      '<div class="meta">' +
        '<span class="l"><b>' + session.done + '</b> von ' + session.total + ' sitzen</span>' +
        '<span class="r">noch ' + session.queue.length + ' offen' +
          (missTotal ? ' <span class="miss">· ' + missTotal + '× falsch</span>' : '') + '</span>' +
        '<button class="reset" id="reset" aria-label="Runde neu starten">↺</button>' +
      '</div>' +
  ...
  // Die Karte lässt sich beliebig oft hin- und herdrehen, bis man sich entscheidet.
